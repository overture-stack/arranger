...
_Shipped 2026-08-10 (#1080), version 1: the scalar operators (`in`, `not-in`, `gt`, `gte`, `lt`, `lte`, `between`) with one `and`/`or` combination per call. **Version 2 shipped 2026-08-25**, adding `wildcard` text search via `fieldNames` plus `some-not-in` and `all`, which brings the tool to full parity with `modules/sqon` except for the unimplemented `fuzzy`. Still deferred: mixed AND/OR nesting (v3) and `fuzzy` (v2.1, blocked on three things listed in § Phasing). The design and implementation records are `.dev/docs/build-sqon-tool.md` and `.dev/docs/build-sqon-implementation.md`._

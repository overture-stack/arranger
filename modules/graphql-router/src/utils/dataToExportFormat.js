...
const getRows = (args) => {
	...
	if (pathIndex >= paths.length - 1) {
		...
		const rowsToFlatten = rows.map((node) => {
			const aRow = getRows({
				...args,
				data: node,
...
const transformDataToTSV = ({ row, uniqueBy = '', columns, valueWhenEmpty }) => {
	...
	const tsvRows = rows.map((r) => rowToTSV({ row: r, valueWhenEmpty }));
...
const transformDataToJSON = ({ enableDebug, row, uniqueBy, columns, valueWhenEmpty }) => {
	try {
...
export default ({
	// TODO: this takes the whole column object
	// and should only take the column name instead
	// then use that into the main fields array
	// which should come from 'ctx'
	columns, // TODO: fix download's column handling!
	ctx = {},
	fileType = 'tsv',
	index,
	uniqueBy,

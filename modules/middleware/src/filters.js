...
export default class FilterProcessor {
  ...
  buildFilters(doc_type, nested, raw_filters) {
    ...
    if (CONSTANTS.RANGE_OPS_KEYS.includes(op))
  ...
  _get_and_filter(doc_type, nested, xs) {
    ...
    xs.forEach(x => {
      ...
      const is_must = CONSTANTS.MUST_OPS.concat(
        CONSTANTS.RANGE_OPS_KEYS,
      ).includes(op);
      ...
      if (is_must || is_must_not) {
        ...
        else if (CONSTANTS.RANGE_OPS_KEYS.includes(op))
  ...
  _get_or_filter(doc_type, nested, content) {
    ...
    return {
      [CONSTANTS.ES_BOOL]:
        should != null || should.length > 0 ? { [ES_SHOULD]: should } : {},
    };
  }
  ...
  wrap_bool(op, val) {
    return {
      [CONSTANTS.ES_BOOL]: { [op]: typeof val === 'object' ? val : [val] },
    };
  }
  wrap_must(val) {
